...
<html>
	...
	<body>
		<sql:setDataSource var="conn" user="root" password="root" driver="com.mysql.cj.jdbc.Driver" url="jdbc:mysql://localhost:3306/p1" />
	
		<sql:query dataSource="${conn}" var="rs">select * from users where username='${param.username}' and password='${param.pwd}';</sql:query>
	
		<c:if test="${rs.rows}">
			<c:out value="Login is Successfull!!!"></c:out>
		</c:if>
